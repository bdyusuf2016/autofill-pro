class PopupManager {
  ...
  applyLocalization() {
    ...
    try {
      ...
      const versionText = chrome.i18n.getMessage("versionText", [manifest.version]);
      if (versionText) {
        document.getElementById("versionText").textContent = versionText;
      }
    } catch (e) {
      console.warn("Error getting manifest version:", e);
    }
  }
  ...
  renderProfileSelect(filter = "") {
    const select = document.getElementById("profileSelect");
    const resultsContainer = document.getElementById("activeSearchResults");
    if (!select) return;
    ...
    const filteredProfiles = profiles.filter((profile) => {
      ...
      const phoneMatch = (profile.phone || profile.phoneNumber || "").toLowerCase().includes(q);
      const nidMatch = (profile.nid || profile.nidNo || profile.nid_no || profile.nationalId || "").toLowerCase().includes(q);
    ...
    // 1. Populate standard select element
    ...
    profiles.forEach((profile) => {
      ...
      if (profile.phone || profile.phoneNumber) {
        labelText += ` (📱 ${profile.phone || profile.phoneNumber})`;
      }
    ...
    // 2. Render Realtime Search Dropdown List
    if (resultsContainer) {
      if (!q) {
        ...
        if (filteredProfiles.length === 0) {
          resultsContainer.innerHTML = `<div style="padding: 10px; color: #666; font-style: italic; text-align: center;">No matching profiles</div>`;
        } else {
          filteredProfiles.forEach((profile) => {
            ...
            left.className = "active-item-left";

            const colorBadge = document.createElement("div");
            colorBadge.className = "active-item-color";
            colorBadge.style.backgroundColor = profile.color || "#4285f4";

            const titleGroup = document.createElement("div");
            const title = document.createElement("span");
            title.className = "active-item-title";
            title.textContent = profile.name;
            titleGroup.appendChild(title);

            const phoneOrNid = profile.phone || profile.phoneNumber || profile.nid || profile.nid_no || profile.nidNo;
            if (phoneOrNid) {
              const sub = document.createElement("span");
              sub.className = "active-item-sub";
              sub.textContent = `📱 ${phoneOrNid}`;
              titleGroup.appendChild(sub);
            }

            left.appendChild(colorBadge);
            ...
            item.addEventListener("click", async () => {
              ...
              this.renderActiveProfileHotkey();
              this.showUrlMatchInfo(false);
              this.showStatus(`Active profile set to: ${profile.name}`);
              resultsContainer.classList.add("hidden");
              const searchInput = document.getElementById("activeProfileSearch");
  ...
  setupEventListeners() {
    // Active profile search input
    const activeSearch = document.getElementById("activeProfileSearch");
    if (activeSearch) {
      activeSearch.addEventListener("input", (e) => {
        this.renderProfileSelect(e.target.value);
      });
    }
  ...
  populateAutofillList(filter = "") {
    ...
    profiles.forEach((profile) => {
      ...
      const phoneNorm = this.normalizeText(profile.phone || profile.phoneNumber || "").toLowerCase();
      const nidNorm = this.normalizeText(profile.nid || profile.nidNo || profile.nid_no || profile.nationalId || "").toLowerCase();
      const fieldNormMatches = (profile.fields || []).some((f) => 
        this.normalizeText(f.name || "").toLowerCase().includes(q) ||
        this.normalizeText(f.value || "").toLowerCase().includes(q)
      );
      if (q && !nameNorm.includes(q) && !phoneNorm.includes(q) && !nidNorm.includes(q) && !fieldNormMatches) return;
      ...
      if (phoneNum) {
        ...
        phoneBadge.style.fontSize = "11px";
        phoneBadge.style.color = "#666";
        phoneBadge.style.marginLeft = "4px";
        phoneBadge.textContent = `📱 ${phoneNum}`;
        left.appendChild(phoneBadge);
      }
